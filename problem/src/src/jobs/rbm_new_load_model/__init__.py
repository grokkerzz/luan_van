...
def analyze(spark, cd):
    ...
    rbm_model = RBM(spark, cd_list=[int(cd)], cd_iter=[1])\
        ...
        .setIterNum(10)\
        .load("dist/data/model/weight.json", "dist/data/model/bv.json", "dist/data/model/bh.json", "dist/data/model/item_mapping.json")\
    ...
    #calculate error
    rmse_evaluator_expectation = RegressionEvaluator(metricName="rmse", labelCol="TKET",
                                    predictionCol="prediction_expectation")
    rmse_evaluator_max = RegressionEvaluator(metricName="rmse", labelCol="TKET",
                                    predictionCol="prediction_max")
    rmse_expectation = rmse_evaluator_expectation.evaluate(predict_test_df)
    rmse_max = rmse_evaluator_max.evaluate(predict_test_df)
    mse_evaluator_expectation = RegressionEvaluator(metricName="mse", labelCol="TKET",
                                    predictionCol="prediction_expectation")
    mse_evaluator_max = RegressionEvaluator(metricName="mse", labelCol="TKET",
                                    predictionCol="prediction_max")
    mse_expectation = mse_evaluator_expectation.evaluate(predict_test_df)
    mse_max = mse_evaluator_max.evaluate(predict_test_df)
    mae_evaluator_expectation = RegressionEvaluator(metricName="mae", labelCol="TKET",
                                    predictionCol="prediction_expectation")
    mae_evaluator_max = RegressionEvaluator(metricName="mae", labelCol="TKET",
                                    predictionCol="prediction_max")
    mae_expectation = mae_evaluator_expectation.evaluate(predict_test_df)
    mae_max = mae_evaluator_max.evaluate(predict_test_df)
    print("Root-mean-square error expectation = " + str(rmse_expectation))
    print("Root-mean-square error max = " + str(rmse_max))
    print("mean-square error expectation = " + str(mse_expectation))
    print("mean-square error max = " + str(mse_max))
    print("mean-absolute error expectation = " + str(mae_expectation))
    print("mean-absolute error max = " + str(mae_max))
