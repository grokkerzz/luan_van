...
from module.collaborative_ultimate_array_rbm import RBM, ProbabilitySoftMaxToExpectationModel, ValueToBinarySoftMaxModel, GroupUserModel
...
def analyze(spark):
    ...
    item_index_mapping = item_index_model.transform(item_df).withColumn("F_MAMH_index", col("F_MAMH_index").cast(LongType())).cache()

    train_input_rbm_df = item_index_model.transform(train_input_rbm_df).withColumn("F_MAMH_index", col("F_MAMH_index").cast(LongType()))
    train_input_rbm_df.printSchema()
