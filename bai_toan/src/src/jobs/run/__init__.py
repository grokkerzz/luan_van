...
from pyspark.sql.functions import udf, col, rank, collect_list, when
import time
...
def get_best_param(spark, train, test_input, test_output, rank_list, ibcf_ranks):
    ...
    for rank in ibcf_ranks:
        result_df = ibcf_model.transform(train_part_df.drop("F_MAMH_index"), validate_part_df.drop("F_MAMH_index"), rank)
        # result_df.show()
    ...
    nbcf_model = NBCFEstimator(spark, user_col, item_col, grade_col, prediction_col)
    ...
    for rank in ibcf_ranks:
        ...
        best_models = put_best_model(best_models, "ubcf",
                                     Model_Error_Wrapper("ubcf_{}".format(rank), nbcf_model, error_nbcf[0], {"rank": rank}))
    ...
    for i in range(len(rank_list)):
        ...
        #
            ...
            predict_als_nn_ibcf_with_gt = predict_als_nn_ibcf.join(test_output, ["MASV1", "F_MAMH"])
...
def run_train(spark, train, test_input, param_dict):
    ...
    nbcf_estimator = NBCFEstimator(spark, user_col, item_col, grade_col, prediction_col)
    nbcf_model = nbcf_estimator.fit(train.unionAll(test_input).drop(item_index_col))
...
def analyze(spark):
    ...
    data_dir = "data/train_val_test_K14_K17"
    # list_faculty = ["MT", "MO", "BD", "CK", "DC", "DD", "GT", "HC", "QL", "UD", "VL", "XD", "VP", "PD"]
    list_faculty = ["GT", "QL", "UD", "VL", "XD", "VP", "PD"]

    # list_faculty = ["VP", "XD"]
    # list_faculty = ["CK", "UD"]
    # list_faculty = ["DC", "QL", "XD"]
    # list_faculty = ["MT"]
    # list_faculty = ["XD"]
    # list_faculty = ["VL", "VP", "XD"]

    model_save_location = "model/{}/{}" #model/faculty/algorithm
    number_of_run = 10
    ...
    for faculty in list_faculty:
        ...
        _, best_param = get_best_param(spark, train_df.union(test_df), validation_input_output[0], validation_input_output[1],rank_list_als,ibcf_ranks)
        ...
        for i in range(number_of_run):
            user_list = all_df.select("MASV1").distinct().rdd.flatMap(lambda x: x).collect()
            ...
            train_df = all_df.join(spark.createDataFrame([[x] for x in train.tolist()], schema=user_schema), ["MASV1"])
            validation_df = all_df.join(spark.createDataFrame([[x] for x in validation.tolist()], schema=user_schema), ["MASV1"])
            test_df = all_df.join(spark.createDataFrame([[x] for x in test.tolist()], schema=user_schema), ["MASV1"])
            ...
            # test_input_output[1] = test_input_output[1].filter(col("TKET") > 1)
            models = run_train(spark, train_df.union(validation_df).cache(), test_input_output[0], best_param_dict)
            ...
            for type_of_model, model in models.items():
                if type_of_model == "als" or type_of_model == "als_nn":
                    result_df = model.transform(test_input_output[1])\
                        .withColumn("prediction", when(col("prediction") < 5.0, 4.0).otherwise(col("prediction")))
                    ...
                    result_df = model.transform(train_part_df.drop("F_MAMH_index"),
                                                validate_part_df.drop("F_MAMH_index"), best_param_dict[type_of_model]["rank"]) \
                        .withColumn("prediction", when(col("prediction") < 5.0, 4.0).otherwise(col("prediction")))
                    # result_df.show()
                    # model.rank = best_param_dict[type_of_model]["rank"]
                    # model.save(model_save_location.format(faculty, type_of_model))
                elif type_of_model == "ubcf":
                    result_df = model.transform(test_input_output[1].drop("F_MAMH_index").drop("TKET"), best_param_dict[type_of_model]["rank"]) \
                        .withColumn("prediction", when(col("prediction") < 5.0, 4.0).otherwise(col("prediction")))\
                        .join(test_input_output[1].drop("F_MAMH_index"), ["MASV1", "F_MAMH"])
                    # model.rank = best_param_dict[type_of_model]["rank"]
                    # model.save(model_save_location.format(faculty, type_of_model))
                else:
                    result_df = model.transform(test_input_output[0], test_input_output[1].drop("TKET")) \
                        .withColumn("prediction", when(col("prediction") < 5.0, 4.0).otherwise(col("prediction"))) \
                        .join(test_input_output[1], ["MASV1", "F_MAMH"])
                    # model.save(model_save_location.format(faculty, type_of_model))
                # result_df.show()
